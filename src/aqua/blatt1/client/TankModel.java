...
public class TankModel extends Observable implements Iterable<FishModel> {
    ...
    private synchronized void updateFishies() {
        for (Iterator<FishModel> it = iterator(); it.hasNext(); ) {
            ...
            if (this.token && fish.hitsEdge() && fish.getDirection().getVector() < 0)
                if (left != null){
                    fishPositions.replace(fish,FishPos.LEFT);
                    forwarder.handOff(fish, left);}
                else fish.reverse();
            if (this.token && fish.hitsEdge() && fish.getDirection().getVector() > 0)
                if (right != null){
                    fishPositions.replace(fish,FishPos.RIGHT);
    ...
    private enum FishPos {HERE, LEFT, RIGHT}
    ...
    public void locateFishGloballyOld(String fishId) {
        ...
        for (FishModel key : fishPositions.keySet()) {
            if(key.getId().equals(fishId)){
                if(fishPositions.get(key).equals(FishPos.HERE)){
                    key.toggle();
